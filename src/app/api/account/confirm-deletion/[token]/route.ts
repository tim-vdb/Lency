...
export async function GET(
    ...
    try {
        ...
        console.log("[confirm-deletion] Received token:", token);

        if (!token) {
            console.log("[confirm-deletion] No token provided");
        ...
        console.log("[confirm-deletion] Confirming deletion...");
        await UsersService.confirmDeleteUser(token);

        console.log("[confirm-deletion] User deleted successfully");
        ...
        if (error instanceof Error) {
            if (error.message === "TOKEN_INVALID_OR_EXPIRED") {
                console.log("[confirm-deletion] Token invalid or expired");
                return NextResponse.redirect(
            ...
            if (error.message === "TOKEN_EXPIRED") {
                console.log("[confirm-deletion] Token expired");
                return NextResponse.redirect(
                    new URL("/?deleted=expired", req.nextUrl.origin),
                    { status: 302 }
                );
